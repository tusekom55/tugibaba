        ...
        SELECT lp.*, 
               ...
               (CASE 
                   WHEN lp.position_type = 'long' THEN 
                       (COALESCE(c.current_price, lp.entry_price) - lp.entry_price) * lp.position_size
                   ELSE 
                       (lp.entry_price - COALESCE(c.current_price, lp.entry_price)) * lp.position_size
...
function openPosition($user_id, $data) {
    ...
    if ($position_type === 'long') {
        $liquidation_price = $entry_price * (1 - ($liquidation_percentage / $leverage_ratio));
    } else {
        $liquidation_price = $entry_price * (1 + ($liquidation_percentage / $leverage_ratio));
    }
...
function liquidatePosition($position_id, $liquidation_price) {
    ...
    // PnL hesapla (genelde büyük zarar)
    if ($position['position_type'] === 'long') {
        $pnl = ($liquidation_price - $position['entry_price']) * $position['position_size'];
    } else {
        $pnl = ($position['entry_price'] - $liquidation_price) * $position['position_size'];
    }
